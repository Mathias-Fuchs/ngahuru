...
static gboolean hlight_searched_strings(GtkSourceBuffer *buffer, gchar *str)
{
...
//	if (direction == 0 || !hlight_check_searched())
	if (direction == 0 || (direction != 2 && !hlight_check_searched()))
	  hlight_searched_strings(GTK_SOURCE_BUFFER (GTK_TEXT_VIEW(textview)->buffer), string_find);
...
//			"replaced", &iter, &match_end);
	  ...
	  hlight_searched_strings(GTK_SOURCE_BUFFER (textbuffer), string_find);
	  hlight_toggle_searched(GTK_SOURCE_BUFFER (textbuffer));
	}
