...
static void cb_changed(GtkSourceBuffer *buffer)
{
	GtkTextIter start, end;
	
	gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER (buffer), &start, &end);
//	gtk_text_buffer_rmvTe_tag_by_name(buffer,
//		"searched", &start, &end);
...
gboolean hlight_toggle_searched(GtkSourceBuffer *buffer)
{
	if (!searched_flag) {
...
void hlight_init(GtkSourceBuffer *buffer)
{
  gtk_text_buffer_create_tag(GTK_TEXT_BUFFER (buffer), "searched",
		"background", "yellow",
		"foreground", "black",
		NULL);
  gtk_text_buffer_create_tag(GTK_TEXT_BUFFER (buffer), "replaced",
		"background", "cyan",
		"foreground", "black",
		NULL);
