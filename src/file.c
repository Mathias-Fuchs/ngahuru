...
gint file_open_real(GtkWidget *view, FileInfo *fi)
{
  ...
  // now enable the syntax highlighting, therefore set up a buffer with the guessed language.
  

	//begin code for language detection
	...
	if (result_uncertain) 
	  content_type = g_content_type_get_mime_type ("text/plain");
	GtkSourceLanguage *lang = gtk_source_language_manager_guess_language (lm, fi -> filename, content_type);
	//GObjectFinalizeFunc (G_OBJECT( lm ));
	g_free (content_type);
	// end code for language detection


	GtkSourceBuffer *buffer = GTK_SOURCE_BUFFER(gtk_text_view_get_buffer(GTK_TEXT_VIEW(view)));

	//begin code enable syntax highlighting
	gtk_source_buffer_set_language(buffer, lang);
	gtk_source_buffer_set_highlight_syntax (buffer, TRUE);
	//end code enable syntax highlighting
...
//	undo_block_signal(buffer);
	...
	gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffer), &iter, str, strlen(str));
	gtk_text_buffer_get_start_iter(GTK_TEXT_BUFFER(buffer), &iter);
