...
void save_config_file(void)
{
	...
	fontname = get_font_name_from_widget(GTK_WIDGET(pub->mw->view));
...
gint main(gint argc, gchar **argv)
{
	...
	conf->width       = 600;
	conf->height      = 400;
	conf->fontname    = g_strdup("Gentium 15");
	conf->wordwrap    = TRUE;
	...
	set_text_font_by_name(GTK_WIDGET(pub->mw->view), conf->fontname);
	...
	gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(
		...
		conf->linenumbers);
	indent_refresh_tab_width(GTK_WIDGET(pub->mw->view));
	gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(
	...
	undo_init(GTK_WIDGET(pub->mw->view),
		gtk_item_factory_get_widget(ifactory, "/Edit/Undo"),
		gtk_item_factory_get_widget(ifactory, "/Edit/Redo"));
//	hlight_init(pub->mw->buffer);
	dnd_init(GTK_WIDGET(pub->mw->view));
	
	if (pub->fi->filename)
	  file_open_real(GTK_WIDGET(pub->mw->view), pub->fi);
#ifdef G_OS_UNIX
	else
...
//		gtk_text_buffer_set_text(buffer, "", 0);
		gtk_text_buffer_get_start_iter( GTK_TEXT_BUFFER (pub->mw->buffer), &iter);
		gtk_text_buffer_insert(GTK_TEXT_BUFFER(pub->mw->buffer), &iter, str, strlen(str));
		gtk_text_buffer_get_start_iter(GTK_TEXT_BUFFER(pub->mw->buffer), &iter);
		gtk_text_buffer_place_cursor(GTK_TEXT_BUFFER(pub->mw->buffer), &iter);
	...
	if (jump_linenum) {
		...
		gtk_text_buffer_get_iter_at_line(GTK_TEXT_BUFFER (pub->mw->buffer), &iter, jump_linenum - 1);
		gtk_text_buffer_place_cursor(GTK_TEXT_BUFFER (pub->mw->buffer), &iter);
//		gtk_text_view_scroll_to_iter(GTK_TEXT_VIEW(textview), &iter, 0.1, FALSE, 0.5, 0.5);
		scroll_to_cursor(GTK_TEXT_BUFFER (pub->mw->buffer), 0.25);
	}
